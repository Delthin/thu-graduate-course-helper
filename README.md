...
**已经装好 Tampermonkey？[点这里一键安装 / 更新脚本](https://raw.githubusercontent.com/Delthin/thu-graduate-course-helper/main/thu-graduate-course-helper.user.js)** · 当前版本 `0.5.7`
...
- 自动加载报名统计：总人数、第一志愿、第二志愿、第三志愿及优先报名人数，并显示估算录取概率。
- 同时为课程表当前显示的课程加载学校上一学年“选课学生推荐度”；优先匹配同课程号，其次匹配同名课，课程改名或教师不匹配时回退按任课教师查询历史汇总。
- 在课程表中直接勾选课程、设置志愿，并同步到原选课表单。
- 将已选定课程显示为蓝色卡片；课程号相同的其他课序，以及上课周次重叠的同时间课程，都会置灰并禁止选择。
...
- **刷新数据**：清除本地缓存，同时重新读取学校发布的报名人数和历史推荐度。
...
报名数据来自学校的“填报志愿情况查询”页面；同一课程课序较多时，脚本会自动读取后续统计分页。脚本会把结果缓存到学校页面显示的“下次统计时间”，如果页面没有给出该时间则缓存四小时；到期后才会重新查询。数据并非每秒更新，手动刷新也不能让学校提前生成新数据。

## 历史推荐度说明

打开课程表后，历史推荐度会与报名人数一起自动加载，课程卡片会显示例如：

```text
同课程推荐 95.7%（95.3分，209人）
```

- `95.7%`：**推荐率**，即历史问卷中选择 6 或 7 分的学生比例。
- `95.3分`：**推荐值**，先计算 1–7 分的加权平均，再将 1 分映射为 0、7 分映射为 100。
- `209人`：该计算使用的历史问卷样本数；将鼠标悬停在文字上可查看 7 分强推荐率。

脚本只查询课程表当前显示的课程，并先按课程名查询；查询结果中课程号相同的记录优先，其次是课程名相同的记录。课程改名或当前教师不在同名课结果中时，会自动回退按教师名查询并显示教师历史汇总。成功结果会按学期保存在浏览器本地，重新打开课程表时直接使用缓存；“暂无历史推荐”不会跨页面缓存，重新进入时会自动复查，点击“刷新数据”也可以强制重查。历史数据只能作参考，课程内容、授课对象和教师当学期安排均可能变化。
...
先查看课程表顶部的人数与推荐查询进度，再点击“刷新数据”。如果状态点变红，应先从信息门户重新进入。
...
- 历史推荐度嵌套表格解析、异步表格稳定等待、6–7 分推荐率、0–100 加权推荐值，以及报名统计与历史推荐同时加载。
- 同课程其他课序禁用、统计刷新时间解析和本地缓存。
- 提交刷新后的已选状态、人数与筛选缓存。
- 登录保活 URL 与过期文本识别。
